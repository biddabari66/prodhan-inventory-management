...
Deno.serve(async (req) => {
    try {
        ...
        if (params.startDate) {
            damagedMovements = damagedMovements.filter(m => new Date(m.movement_date || m.created_date) >= new Date(params.startDate));
        }
        if (params.endDate) {
            damagedMovements = damagedMovements.filter(m => new Date(m.movement_date || m.created_date) <= new Date(params.endDate));
        }
